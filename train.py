...
def train_one_epoch(__C,
    ...
    meters = [batch_time, data_time, losses,losses_seg,lr]
    ...
    for ith_batch, data in enumerate(loader):
        ...
        if scalar is not None:
            with th.cuda.amp.autocast():
                loss = net(image_iter,lang_iter,det_label=box_iter,seg_label=mask_iter)
        else:
            loss = net(image_iter, lang_iter, det_label=box_iter,seg_label=mask_iter)
...
def main_worker(gpu,__C):
    global best_det_acc,best_seg_acc,best_oiou_acc
    best_det_acc,best_seg_acc,best_oiou_acc=0.,0.,0.
    if __C.MULTIPROCESSING_DISTRIBUTED:
    ...
    for ith_epoch in range(start_epoch, __C.EPOCHS):
        ...
        box_ap,mask_ap, oiou=validate(__C,net,val_loader,writer,ith_epoch,gpu,val_set.ix_to_token,save_ids=save_ids,ema=ema)
        if main_process(__C,gpu):
            if ema is not None:
                ema.apply_shadow()
            ...
            if oiou>best_oiou_acc:
                best_oiou_acc=oiou
                torch.save({'epoch': ith_epoch + 1, 'state_dict': net.state_dict(), 'optimizer': optimizer.state_dict(),
                            'scheduler': scheduler.state_dict(),'lr':optimizer.param_groups[0]["lr"],},
                           os.path.join(__C.LOG_PATH, str(__C.VERSION),'ckpt', 'oiou_best.pth'))
